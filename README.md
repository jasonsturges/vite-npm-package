...
Using the `start` task, Vite hosts the `index.html` for a local development environment.  This file is not included in the production build.  Note that only specified exports from the `index.js` are ultimately bundled into the library.
